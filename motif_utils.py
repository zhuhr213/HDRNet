    ...
    **kwargs -- other input arguments
    ...
    if return_idx:
        ...
        return [motifs[i] for i, pval in enumerate(pvals) if pval < cutoff], \
            {motifs[i]:pval for i, pval in enumerate(pvals) if pval<cutoff}
...
def motif_analysis(pos_seqs,
    ...
    **kwargs -- other input arguments, may include:
    ...
    motifs_to_keep, motif_pval = filter_motifs(pos_seqs, 
                                   neg_seqs, 
    ...
    motif_pval_keep = {}
    for key in merged_motif_seqs:
        motif_pval_keep[key] = motif_pval[key]


    motif_pval_keep_ = sorted(motif_pval_keep.items(), key=lambda x:x[1])[0:3]
    # print(motif_pval_keep)

    kept_seq = [seq[0] for seq in motif_pval_keep_]
    # print(kept_seq)
    # print(kept_seq)

    # pos_index这里保存了dev.tsv的index，可以通过这个找到二级结构信息
    import pandas as pd
    structs_dir = data_dir + '/dev_struc.tsv'
    structs = pd.read_csv(structs_dir, sep='\t', header=0)
    
    if save_file_dir is not None:
        if verbose:
            print("* Saving outputs to directory")
        os.makedirs(save_file_dir, exist_ok=True)
        for motif, instances in merged_motif_seqs.items():  # 这个东西是dict
            if motif not in kept_seq:
                continue
            # saving to files
            ...
            with open(save_file_dir+'/motif_{}_{}.txt'.format(motif, len(instances['seq_idx'])), 'w') as f:
                ...
                for seq in instances['seqs']:
                    struc = structs.loc[pos_index[instances['seq_idx'][i]]][0].split(',')
                    range = instances['atten_region_pos'][i]
                    struc = struc[range[0]:range[1]]
                    struc = ','.join(struc)
                    ind = str(pos_index[instances['seq_idx'][i]])
                    range = str(range)
                    f.write(seq+'\t'+struc+'\t'+ind+'\t'+range+'\n')
                    i+=1
